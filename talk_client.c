...
#include "speak2.h"
    ...
    /* this socket os to setup/teardown a call and parameters only*/
    printf("remote port is %d\n", myport);
    server.sin_family = AF_INET;
    server.sin_port = htons(myport);
    server.sin_addr.s_addr = INADDR_ANY;
    /* this is the call socket for sending recieving audio*/
	...
	/*if remote host accepts send bit rate and chunk size*/
        printf("got accept..\n");
	...
	memset(buffer, 0, sizeof(buffer)); 
        ...
        /* wait for an acknowledgement of the parameters */ 
        recv_no = recv(mysocket, buffer, sizeof(buffer),0);
    }
